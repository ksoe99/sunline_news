    ...
    <Card className={`${feature ? "md:col-span-2" : ""} overflow-hidden`}>
      <div className="grid grid-cols-1 md:grid-cols-2">
        <div className="relative aspect-[16/9] w-full overflow-hidden bg-gray-100">
          <img
            src={article.image || "/assets/placeholders/16x9.jpg"}
            alt=""
        ...
        <div className="flex flex-col gap-3 p-4 md:p-6">
          <a href="#" className="group">
            <h3
              className="font-serif text-2xl font-extrabold leading-tight tracking-tight md:text-3xl"
              style={{ fontFamily: "var(--brand-heading)" }}
            >
              {article.title}
            </h3>
...
function ArticlePage({ article }: { article: Article }) {
  return (
    <main className="mx-auto max-w-3xl px-4 py-8 lg:px-0">
      <h1
        className="mb-3 font-serif text-4xl font-black leading-tight"
        style={{ fontFamily: "var(--brand-heading)" }}
      >
        {article.title}
      </h1>
    ...
    <footer className="mt-10 border-t border-gray-100 bg-white">
      <div className="mx-auto max-w-6xl px-4 py-8 text-sm text-gray-600 lg:px-6">
        <p className="mt-6 text-xs text-gray-400">
          © {new Date().getFullYear()} Sunline News. Part of the Sunline Network.
        </p>
      </div>
    </footer>
  );
...
const mock: Article[] = Array.from({ length: 3 }).map((_, i) => ({
  ...
  summary:
    "Concise dek that previews the article and sets reader expectations.",
...
export default function SunlineKit({
  brand = "sunline",
  articles = mock,
}: {
  brand?: string; // broadened to string
  articles?: Article[];
}) {
  // Map any incoming string to a valid theme key. Fallback to 'sunline'.
  const isValid =
    typeof brand === "string" &&
    Object.prototype.hasOwnProperty.call(themes, brand as string);
  const safeBrand = (isValid ? (brand as BrandKey) : "sunline") as BrandKey;

  const [view] = useState<"home" | "article">("home");

  return (
    <ThemeProvider brand={safeBrand}>
      <Header />
      {view === "home" ? (
